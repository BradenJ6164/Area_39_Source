...
function region:castwhite(white, maxParts)
	local white = type(white) == "table" and white or {white};
	local maxParts = maxParts or 20; -- 20 is default for normal region3
	...
	-- cast non-rotated region first as a probe
	local realRegion3 = Region3.new(rmin, rmax);
	local parts = game.Workspace:FindPartsInRegion3WithWhiteList(realRegion3, white, maxParts);
