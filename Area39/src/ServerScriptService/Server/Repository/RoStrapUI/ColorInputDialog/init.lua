...
local Frame do
	...
	Background.Size = UDim2.new(0.2, 505, .2, 505)
	...
	local HeadTxt = Instance.new("StringValue")
	HeadTxt.Name = "HeadTxt"
	HeadTxt.Parent = Header
...
local function OnConfirm(self)
	if not self.Dismissed then
		self:Dismiss()
		self.OnConfirmed:Fire(LocalPlayer, self.Object.Background.colorPick.SelectedColor.Value)
	end
end
...
return PseudoInstance:Register("ColorInputDialog", {
	Storage = {};
	...
	Properties = {
		...
		HeaderText = Typer.AssignSignature(2, Typer.String, function(self, Text)
			self.Header.HeadTxt.Value = Text
	...
	Init = function(self, ...)
		...
		local TextBox = script.colorPick:Clone()
		TextBox.Size = UDim2.new(0,505,0,505)
		TextBox.Position = UDim2.new(0.5, 0, 0.5, 0)
		TextBox.AnchorPoint = Vector2.new(0.5,0.5)
		TextBox.ZIndex = 3
		TextBox.Parent = self.Background
		TextBox.SelectedColor.Changed:Connect(function()	
			self.Object.Background.Header.Text = self.Header.HeadTxt.Value .. " | " .. tostring(TextBox.SelectedColor.Value)
