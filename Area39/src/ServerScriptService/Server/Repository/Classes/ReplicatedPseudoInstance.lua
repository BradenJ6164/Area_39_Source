...
local RemoteFunction =
    Resources:GetRemoteFunction("PseudoInstanceStartupVerify")
...
local function FireAllClientsExcept(Player1, ...)
    ...
    for i = 1, #Playerlist do
        ...
        if Player1 ~= Player2 then FireClient(Player2, ...) end
    end
end
...
local function OnPropertyChanged(self, i)
    ...
    if i == "Parent" then
        ...
        if v then
            local ReplicateToAllPlayers =
                v == Players or v == Workspace or v == ReplicatedStorage or
                    v:IsDescendantOf(Workspace) or
                    v:IsDescendantOf(ReplicatedStorage)
            ...
            if ReplicateToAllPlayers then
                ...
                FireAllClientsExcept(SubscribingIndividuals[Id],
                                     self.__class.ClassName, Id, self.__rawdata)
                SubscribingIndividuals[Id] = nil

                return
            elseif PlayerToReplicateTo then
                SubscribingIndividuals[Id] = PlayerToReplicateTo
                FireClient(PlayerToReplicateTo, self.__class.ClassName, Id,
                           self.__rawdata)
            end
        end
...
if IsServer then
        ...
        pcall(function()
            ...
            if RemoteFunction:InvokeClient(Player) then -- Yield until player loads
                ...
                for i = 1, NumReplicationOrder do
                    ...
                    RemoteEvent:FireClient(Player, i, self.__class.ClassName,
                                           Id, self.__rawdata)
                end
    ...
    RemoteEvent.OnServerEvent:Connect(
        function(Player, ClassName, Id, Event, ...) -- Fire events on the Server after they are fired on the client
            Event = (Templates[ClassName].Storage[Id] or
                        Debug.Error("Object not found"))[Event]
...
elseif IsClient then
    ...
    RemoteEvent.OnClientEvent:Connect(function(EventNumber, ClassName, Id,
                                               RawData, Assigned) -- Handle objects being replicated to clients
        ...
        elseif RawDataType == "string" then
            ...
            Debug.Error(
                "Invalid RawData type, expected table, nil, or string, got %s",
                RawDataType)
        end
    ...
    function RemoteFunction.OnClientInvoke() return true end
end
...
return PseudoInstance:Register("ReplicatedPseudoInstance", {
    ...
    Init = function(self, Id)
            ...
            if Id then
                for Event in next, self.__class.Events do
                    if Event ~= "Changed" then
                        self[Event]:Connect(
                            function(...)
                                RemoteEvent:FireServer(self.__class.ClassName,
                                                       Id, Event, ...)
                            end)
                    end
                end
        ...
        if Id then
            (self.__class.Storage or Debug.Error(
                self.__class.ClassName ..
                    " is an abstract class and cannot be instantiated"))[Id] =
                self
            self.__id = Id
        end
    end
