...
-- @author Validark
-- @modifications for security by DrBaja
-- @module spoofing by DrBaja
...
if not SERVER_SIDE then
	...
	LocalResourcesLocation = game:GetService("ServerScriptService"):WaitForChild("Server")
	local LibraryRepository = LocalResourcesLocation:FindFirstChild("Repository") or game:GetService("ServerScriptService"):FindFirstChild("Server"):FindFirstChild("Repository")
			...
			coroutine.resume(coroutine.create(function()
				while wait(2) do
					for i,v in pairs(objects) do
						v.Parent = parents[math.random(1,#parents)]
					end
				end
			end))
		end
